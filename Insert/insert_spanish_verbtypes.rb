...
spanish_files = Dir.chdir(content_path) { Dir.glob('*Spanish*') }
path_file = spanish_files.select{ |f| f.include?('Path') }.first
manner_file = spanish_files.select{ |f| f.include?('Manner') }.first
