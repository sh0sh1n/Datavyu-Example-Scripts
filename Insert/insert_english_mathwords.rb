...
content_file = '~/Desktop/English Word List.csv'
...
english_mathwords = new_column('english_mathwords','words')
...
unless get_column_list.include?('english_categories')
  # intialize a new column to store word categories in as well
  english_categories = new_column('english_categories',%w[number shapes mag_comp
    loc_dir or deictics feat_prop pattern time false generic])
  else
    english_categories = get_column('english_categories')
end
...
set_column('english_mathwords', english_mathwords)
set_column('english_categories', english_categories)
